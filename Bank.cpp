...
#include <fstream>
#include <cstring>
#include <conio.h>   // ✅ Windows equivalent of <termios.h>
#include <ctime>
using namespace std;

// ✅ Delay function for better UX
...
void readPassword(char* buffer, int len = 4) {
    ...
    for (int i = 0; i < len; i++) {
        buffer[i] = _getch();   // ✅ works on Windows
        cout << '*';
        delay(100);
    }
    buffer[len] = '\0';
...
public:
    ...
    static void showAccount() {
        ...
        if (authenticate(n, b1)) {
            cout << "Account Number: " << b1.acno << endl
                 << "Name: " << b1.name << endl
                 << "Amount: " << b1.amt << endl
                 << "Mobile: " << b1.mob << endl
                 << "Status: " << b1.status << endl
                 << "Type: " << b1.actype << endl;
        }
    }
